...
export const MissionsHeader = (props: MissionHeaderProps) => {
  const [selectedMovement, setSelectedMission] = useState(0);
  const [childData, setChildData] = useState<number>(0);
  const sendDataToParent = (e: number) => {
    props.onDataReceived(e);
  };

  const handleSelectMission = (e: number) => {
    switch (e) {
      case 0:
    ...
    <div>
      <div className="navbar bg-white">
        <div className="navbar-start">
          <div
            className={selectedMovement === 0 ? "mission-active" : "mission"}
            onClick={() => [handleSelectMission(0), sendDataToParent(0)]}
          >
            مهمة قيد الإنتظار
          </div>
          <div
            className={selectedMovement === 1 ? "mission-active" : "mission"}
            onClick={() => [handleSelectMission(1), sendDataToParent(1)]}
          >
            مهمة تحت التنفيذ
          </div>
          <div
            className={selectedMovement === 2 ? "mission-active" : "mission"}
            onClick={() => [handleSelectMission(2), sendDataToParent(2)]}
          >
            مهمة متأخرة
          </div>
          <div
            className={selectedMovement === 3 ? "mission-active" : "mission"}
            onClick={() => [handleSelectMission(3), sendDataToParent(3)]}
          >
            مهمة تامة تنتنظر التقييم
          </div>
          <div
            className={selectedMovement === 4 ? "mission-active" : "mission"}
            onClick={() => [handleSelectMission(4), sendDataToParent(4)]}
          >
            مهمة تامة
          </div>
        </div>
